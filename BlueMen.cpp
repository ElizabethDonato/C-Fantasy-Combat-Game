...
int BlueMen::defend()
{
   ...
   {
	num = 1;
	std::cout << "The Blue Men's defenses are weakened.  Only one defense die remains." << std::endl << std::endl;
   }

   if((strength > 4) && (strength < 9))
   {
	num = 2;
	std::cout << "The Blue Men's defenses are weakened.  Only two defense dice remain." << std::endl << std::endl;
   }
   totalRoll = roll(num, size);
